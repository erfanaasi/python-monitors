...
class PastMTLBuilder(PastMTLVisitor):
    ...
    def __init__(self):
        super(PastMTLVisitor, self).__init__()
        self.num = 0
        self.name = "Monitor"
    ...
    def visitTimedSinceInf(self, ctx):
        left = self.visit(ctx.left)
        right = self.visit(ctx.right)
    ...
    def visitTimedAlwaysInf(self, ctx):
        child = self.visit(ctx.child)
        label = "self.states[{}]".format(self.num)
    ...
    def visitTimedOnceInf(self, ctx):
        child = self.visit(ctx.child)
        label = "self.states[{}]".format(self.num)
